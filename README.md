...
1. 进入「语音预设」页面
2. 点击「新建预设」（默认为语音克隆模式）
3. 上传参考音频：选择文件或直接录音（最长 10 秒）
4. 系统自动使用 ASR 模型转录参考文本（可手动修改）
5. 音频自动截取前 30 秒
6. 保存预设后即可在转换时使用
...
├── ios/
│   ├── xReader.xcodeproj        # Xcode 项目
│   └── xReader/
│       ├── App.swift            # 入口 + TabView
│       ├── Models/              # Codable 数据模型
│       ├── Network/             # API 客户端
...
选择模拟器或设备后 Cmd+R 运行。首次启动输入后端服务器地址。

### 功能

- 首次启动输入服务器地址，支持连接验证
- 认证登录（Challenge-Response + JWT）
- 图书列表、上传、详情、章节管理
- 章节转换：单章 / 选中 / 全部，实时进度显示，完成后自动刷新
- 语音预设管理：创建 / 编辑 / 删除，支持语音设计和语音克隆
- 语音克隆参考音频：上传文件或直接录音（10 秒）
- 编辑预设时测试语音效果，生成后在线播放
- 音频播放器，支持锁屏控制和后台播放
- 系统配置查看
