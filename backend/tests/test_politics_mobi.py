...
class TestPoliticsMobi:
    ...
    def test_total_chapters(self, parsed):
        """Should have 13 chapters: 序 + 前言 + 5 lectures + 总论 + 附录"""
        assert len(parsed["chapters"]) == 13

    def test_chapter_titles(self, parsed):
        titles = [ch["title"] for ch in parsed["chapters"]]
        assert titles == [
            "序", "前言",
            "第一讲·汉代", "第二讲·唐代", "第三讲·宋代", "第四讲·明代", "第五讲·清代",
            "总论", "钱穆年表", "钱穆先生小传", "注释",
            "读《中国历代政治得失》及其它——王昆吾",
    ...
    def test_chapter_content_not_empty(self, parsed):
        for ch in parsed["chapters"]:
            assert ch["word_count"] > 100, f"Chapter '{ch['title']}' only has {ch['word_count']} chars"
