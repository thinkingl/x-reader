...
修改任何代码文件前，必须完成以下 checklist，不可跳过任何步骤：

1. [ ] **修改代码**：完成所需的功能/修复
2. [ ] **补充单元测试**：如果该问题没有对应的单元测试，新增测试覆盖
3. [ ] **运行全部单元测试**：执行 `pytest tests/ -v --tb=short`，确认全部通过
4. [ ] **部署生效**：将修改的文件复制到容器并重启
5. [ ] **验证运行正常**：确认服务正常响应

## 工作约束

- **修改代码 → 单元测试 → 部署生效**：严格按此顺序执行，不可跳过测试直接部署。
