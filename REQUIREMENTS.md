...
- [x] 技术栈：React + Ant Design（Web）
- [x] 技术栈：SwiftUI（iOS 原生客户端）
- [x] 响应式布局
...
**核心需求：**
- 支持可选的 API 认证，默认不启用
- 认证启用时，所有 API 请求需要携带有效的 Token
- 停用认证时需要验证当前 key（防止未授权关闭）
- 启用认证时可以设置新 key（无需验证）
- 网络上传输不暴露明文 key

**认证方式：** Challenge-Response + JWT Token
...
│  1. 用户输入 auth_key                                                 │
│  2. 前端生成 salt (随机16字节)                                        │
│  3. 前端计算 stored_hash = SHA256(salt + auth_key)                   │
│  4. PUT /api/config {auth_key_hash, auth_key_salt, auth_enabled}    │
│  5. 后端存储 hash 和 salt，不存储明文                                 │
│                                                                       │
│  场景二：登录认证                                                      │
│  ────────────────────                                                │
│  1. POST /api/auth/challenge → 后端返回 {nonce, timestamp, salt}    │
│  2. 前端计算 response = HMAC-SHA256(auth_key, nonce + timestamp)    │
│  3. POST /api/auth/verify {response, timestamp}                     │
│  4. 后端验证 HMAC，通过后返回 JWT Token                               │
│  5. 后续请求携带 Authorization: Bearer <token>                       │
│                                                                       │
│  场景三：停用认证（需要校验）                                          │
│  ─────────────────────────────                                       │
│  1. POST /api/auth/challenge → 获取 challenge                       │
│  2. 前端计算 response = HMAC-SHA256(auth_key, nonce + timestamp)    │
│  3. POST /api/auth/disable {response, timestamp}                    │
│  4. 后端验证通过后设置 auth_enabled = false                          │
│                                                                       │
│  场景四：停用状态启用认证                                              │
│  ─────────────────────────                                           │
│  1. 用户输入新 auth_key                                               │
│  2. 前端计算 hash + salt                                              │
│  3. POST /api/auth/enable {hash, salt}                               │
│  4. 无需验证（因为当前认证已停用）                                    │
│                                                                       │
└─────────────────────────────────────────────────────────────────────┘
```
...
| 特性 | 实现方式 |
|------|----------|
| 明文 key 不传输 | Challenge-Response 模式 |
| 防重放攻击 | nonce + timestamp，60秒有效期 |
| 防彩虹表攻击 | 随机 salt |
...
- auth_key_hash: SHA256(salt + auth_key) 结果
- auth_key_salt: 随机16字节的 base64 编码
```
...
# 需要认证的接口
POST /api/auth/disable       # 停用认证（需要验证 key）
PUT  /api/config             # 更新配置
其他所有 API
```
...
采用 Challenge-Response + JWT Token 认证方案，核心特性：
1. **明文 key 不传输**：使用 HMAC-SHA256 签名验证
2. **防重放攻击**：nonce + timestamp，60秒有效期
3. **防彩虹表攻击**：使用随机 salt
4. **停用认证需验证**：防止未授权关闭认证
5. **启用认证无需验证**：因为之前没有 key
